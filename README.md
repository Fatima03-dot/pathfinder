...
Guaranteed optimality: It will always find the shortest path.

Reliable: Works effectively even in complex, trap-heavy environments.

Cons:

Higher memory usage: Must track g(n) scores for many nodes.

Slower in open spaces: Visits more nodes to verify optimal pathing.

Greedy Best-First Search (GBFS)
Pros:

Speed: Typically expands fewer nodes and finds "a" solution faster than A* in simple scenarios.

Low computational overhead: Does not need to calculate or update path costs (g).

Cons:

Sub-optimal: The path found is often much longer than necessary.

Trap-prone: Can easily get stuck in dead-ends or "U-shaped" obstacles because it lacks a sense of actual distance traveled.
